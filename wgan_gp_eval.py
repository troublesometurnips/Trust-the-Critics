...
if args.FID:
    ...
    for g_idx in range(1+(num_gen-1)//args.eval_freq):
        ...
        fake_mean, fake_cov = fid_score.compute_statistics_of_path(os.path.join(temp_dir, 'pics/timestamp{}'.format(gen_num)), inception_net, 100, 2048, device)
        fidvals[g_idx] = fid_score.calculate_frechet_distance(fake_mean, fake_cov, real_mean, real_cov)
